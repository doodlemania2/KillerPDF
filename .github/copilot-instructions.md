...
KillerPDF is a Windows-only WPF PDF editor distributed as a single, portable, self-contained `KillerPDF.exe`. The repo now targets `net9.0-windows`; building and publishing require Windows plus the .NET 9 SDK.
...
- Publish (single-EXE bundle): `dotnet publish -c Release -r win-x64 -p:PublishSingleFile=true -p:SelfContained=true` → output in `bin/Release/net9.0-windows/win-x64/publish/KillerPDF.exe`.
  ...
  - Publish uses SDK single-file properties in `KillerPDF.csproj` (`PublishSingleFile`, `SelfContained`, `IncludeNativeLibrariesForSelfExtract`, and compression enabled).
...
The repo lives on macOS in this environment but the project **only builds on Windows** (`UseWPF=true`, `net9.0-windows`, `win-x64`, native `pdfium.dll` P/Invoke). Do not expect `dotnet build` to succeed here; rely on careful reading instead of compile checks, or hand work off to a Windows runner with the .NET 9 SDK.
...
- Versioning: bump `<Version>` in `KillerPDF.csproj`; the SDK auto-generates assembly metadata from it unless `<GenerateAssemblyInfo>` is explicitly disabled. If explicit `<AssemblyVersion>`/`<FileVersion>` values remain, keep them in sync. Add a `## [x.y.z] - YYYY-MM-DD` section to `CHANGELOG.md` (Keep a Changelog format, SemVer), and update the compare links at the bottom.
